...
#include <opm/simulators/flow/CompositionalEquilInitializer.hpp>
...
protected:
    ...
    void readEquilInitialCondition_() override
    {
        ...
        // computed via flash-based compositional equilibration.
        CompositionalEquilInitializer<TypeTag> equilInitializer(simulator);

        const std::size_t numElems = this->model().numGridDof();
        initialFluidStates_.resize(numElems);
        for (std::size_t elemIdx = 0; elemIdx < numElems; ++elemIdx) {
            initialFluidStates_[elemIdx] = equilInitializer.initialFluidState(elemIdx);
        }
