...
*/
#ifndef OPM_MAIN_SEQ_HEADER_INCLUDED
#define OPM_MAIN_SEQ_HEADER_INCLUDED

#include <flow/flow_seq.hpp>
...
namespace Opm
{
    ...
    private:
        int dispatchDynamic_()
        {
            ...
            if( phases.size() == 3 ) {
                Opm::flowSeqSetDeck(setupTime_, deck_.get(), *eclipseState_, *schedule_, *summaryConfig_);
                return Opm::flowSeqMain(argc_, argv_, outputCout_, outputFiles_);
            }
            else {
                if (outputCout_)
