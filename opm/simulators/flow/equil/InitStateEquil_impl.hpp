...
operator()(const Scalar x) const
{
    ...
    // Handling of an evaluation point outside "span_": the interval is clamped
    // to the table, and the interpolation parameter with it, so that a depth
    // beyond either end returns the value at that end.  Deriving "t" from the
    // unclamped index instead evaluated a point exactly at the end of the span
    // at t = 0 of the final interval, i.e. one integration step early.
    if (i  <  0) { i = 0;      }
    if (N_ <= i) { i = N_ - 1; }

    const Scalar t = std::clamp((x - (span_[0] + i*h)) / h, Scalar{0}, Scalar{1});
