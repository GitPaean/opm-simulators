        ...
        case WellEconProductionLimits::EconWorkover::CONP:
            {
                ...
                this->closeOffendingConnections(ratio_report.worst_offending_completion,
...
template<typename Scalar, typename IndexTraits>
void WellTest<Scalar, IndexTraits>::
closeOffendingConnections(const int worst_offending_completion,
                          const bool close_connections_below,
                          const double simulation_time,
                          const bool write_message_to_opmlog,
                          WellTestState& well_test_state,
                          DeferredLogger& deferred_logger) const
{
    ...
    if (!close_connections_below) {
        completions_to_close.push_back(worst_offending_completion);
    } else {
        completions_to_close.push_back(worst_offending_completion);
        bool below_worst_offender = false;
        for (const auto& connection : connections) {
            if (!below_worst_offender &&
                connection.complnum() == worst_offending_completion) {
