...
namespace Opm
{
         ...
         const int num_cells)
    {
        ...
        if (primary_variables_[WFrac] < 0. || primary_variables_[GFrac] < 0. ) {
            std::cout << " fraction is smaller that zero for well " << name() << std::endl;
        }
    }
                    ...
                    Opm::DeferredLogger& deferred_logger) const
    {
        ...
        std::cout << " well " << name() << " perf " << perf << " reservoir pressure " << pressure.value()/1.e5 << " perforation pressure " << well_pressure.value()/1.e5
                  << " rates " << cq_s[0].value()*86400. << " " << cq_s[1].value()*86400.
                  << " " << cq_s[2].value()*86400. << std::endl;
    }
    ...
    updateWellStateFromPrimaryVariables(WellState& well_state, Opm::DeferredLogger& deferred_logger) const
    {
        ...
        // TODO: it is not correct, while for debugging purpose, we use this if condition
    ...
    updateThp(WellState& well_state, Opm::DeferredLogger& deferred_logger) const
    {
        ...
        if (well_state.thp()[index_of_well_] < 1.0e6) {
            std::cout << " well " << name() << " thp is " << well_state.thp()[index_of_well_] << std::endl;
            return;
        }
    }
                          ...
                          Opm::DeferredLogger& deferred_logger)
    {
        ...
        if ( !this->operability_status_.isOperable() ) {
            return;
        }
    ...
    checkOperabilityUnderTHPLimitProducer(const Simulator& /* ebos_simulator */, Opm::DeferredLogger& deferred_logger)
    {
        ...
        std::cout << " well " << name() << " obtained bhp is " << obtain_bhp/1.e5 << " during operability checking " << std::endl;
                                            ...
                                            Opm::DeferredLogger& deferred_logger) const
    {
        ...
        std::vector<double> rates_from_ipr(number_of_phases_, 0.);

        for (int p = 0; p < number_of_phases_; ++p) {
            rates_from_ipr[p] = bhp * ipr_b_[p] - ipr_a_[p];
            well_state.wellRates()[index_of_well_ * number_of_phases_ + p] = bhp * ipr_b_[p] - ipr_a_[p];
        }
        ...
        // TODO: when crossing flow happens, the result is far from converged. Here, we should solve it until converged
        // to see whether we can get better result.

        // TODO: it also means, when crossflow happens, the initial guess from the intersection can be very bad.

        // TODO: to be safe, we should explore the brute-force technique in a relatively efficient way

        for (int p = 0; p < 3; ++p) {
            if (rates[p] > 0.) {
                std::cout << " well rates for well " << name() <<  " " << rates[0] << " " << rates[1] << " " << rates[2] << std::endl;
                break;
            }
        }
        ...
        std::cout << " rates_from_ipr with bhp " << bhp/1.e5  << " " << rates_from_ipr[0] << " " << rates_from_ipr[1] << " " << rates_from_ipr[2] << std::endl;

        std::cout << " rates " << rates[0] << " " << rates[1] << " " << rates[2] << std::endl;
    ...
    updatePrimaryVariables(const WellState& well_state, Opm::DeferredLogger& deferred_logger) const
    {
        ...
        if (std::abs(total_well_rate) > 0.) {
            const auto pu = phaseUsage();
            if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
                const int index_water = pu.phase_pos[Water];
